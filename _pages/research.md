...
6. **Speeding up Kernel PCA (KPCA)** (July '17 - Oct '17) <br/>
    ...
    * Used the **improved Nyström method** to obtain a **low rank** approximation to the Gram matrix. Using this,
      developed a **fast algorithm for eigenvector computation** in KPCA, **improving time complexity** from $O(x)$
      to $O(y)$, where n is the number of data points and p << n is the rank of the approximated Gram matrix. <br/>
